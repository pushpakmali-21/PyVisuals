...
import { ArrowLeft, Database, LineChart, Play, RefreshCw, Table2 } from "lucide-react";
...
export function PyVisualsIde() {
  ...
  const runLive = useCallback(async (source: string) => {
    ...
    try {
      ...
      setTerminalLines(
        result.terminal.length
          ? result.terminal
          : [{ stream: "stdout", text: "Program completed with no output.\n" }]
      );
    } catch (error) {
      setTerminalLines([
        {
    ...
    <main className="flex h-screen min-h-[720px] flex-col overflow-hidden bg-[#0c1017] text-slate-100">
      ...
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-line bg-[#111722] px-4">
        <div className="flex min-w-0 items-center gap-3">
          <Link
            href="/"
            className="flex h-8 w-8 items-center justify-center rounded-md border border-line bg-panelSoft text-slate-400 hover:border-accent hover:text-accent transition-colors"
            title="Back to Home"
          >
        ...
        <div className="flex items-center gap-2">
          <button
            className="flex h-9 items-center gap-2 rounded-md border border-line bg-panelSoft px-3 text-sm text-slate-200 hover:border-accent disabled:cursor-not-allowed disabled:opacity-60 transition-colors"
          ...
          <button
            className="flex h-9 items-center gap-2 rounded-md border border-line bg-panelSoft px-3 text-sm text-slate-200 hover:border-accent transition-colors"
          ...
          <button
            className="flex h-9 items-center gap-2 rounded-md bg-accent px-4 text-sm font-semibold text-ink hover:bg-[#78eee4] disabled:cursor-not-allowed disabled:opacity-60 transition-colors"
      ...
      {/* Main body with resizable horizontal splitter */}
      <ResizableSplitter
        direction="horizontal"
        storageKey="pyvisuals-editor-width"
        initialSize={560}
        minFirst={320}
        minSecond={360}
      >
        {/* Left: Code Editor */}
        <section className="flex h-full min-h-0 flex-col border-r border-line bg-[#10151d]">
        ...
        {/* Right: Visualizer + Terminal with vertical splitter */}
        <ResizableSplitter
          direction="vertical"
          storageKey="pyvisuals-visualizer-height"
          initialSize={400}
          minFirst={180}
          minSecond={140}
        >
